...
## Algoritmo Greedy - Earliest finish time first
```Pseudo
1. ordino i j jobs ib base al loro finish time
2. inizializzo il sottoinsieme S
3. scorro i jobs dal primo all'ultimo
	- se il job che sto vedendo é compatibile con S
		- allora inserisco il job all'interno di S
	- Altrimenti no
4. Restituisco S
```
...
Siano $j_{1},\dots, j_{m}$ l'insieme di job appartenenti all'algoritmo ottimo
